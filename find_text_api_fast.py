...
# -*- coding: utf-8 -*-
"""
Created on Fri May 20 14:15:25 2022
        ...
        for adress in adresses:
            ...
            if(pincode!=""):
                if(street.lower() in content.lower() and content.lower().index(street.lower())>10 and  ("Order of" not in  content[content.lower().index(street.lower())-10:content.lower().index(street.lower())] or "Order of" not in street)):
                    ...
                    latitude=""
                    longitude=""
                    address_list.append([street,latitude,longitude,city,state,pincode])
                    ...
                    for k in parsed_text:
                        ...
                        if(k in content[content.index(i)-100:content.index(i)]):
                            ...
                            if(address_find.lower() in content.lower() and content.lower().index(address_find.lower())>10 and  ("Order of" not in  content[content.lower().index(address_find.lower())-10:content.lower().index(address_find.lower())] or "Order of" not in address_find)):
                                ...
                                latitude=""
                                longitude=""
                                address_list.append([address_find,latitude,longitude,city_find,state_find,i])
                    for o in po_box_search:
                ...
                for nn in door_number_search:
                    if(content.index(i)-100>=0 and nn in content[content.index(i)-100:content.index(i)]): 
                        ...
                        if(address_Drnum_find.lower() in content.lower() and content.lower().index(address_Drnum_find.lower())>10 and  ("Order of" not in  content[content.lower().index(address_Drnum_find.lower())-10:content.lower().index(address_Drnum_find.lower())] or "Order of" not in address_Drnum_find)):
                            ...
                            address_list.append([address_Drnum_find,latitude,longitude,city_find,state_find,i])
                for pp in  po_box_search:
                    if(content.index(i)-100>=0 and pp in content[content.index(i)-100:content.index(i)]): 
                        ...
                        if(address_pb_find.lower() in content.lower() and content.lower().index(address_pb_find.lower())>10 and  ("Order of" not in  content[content.lower().index(address_pb_find.lower())-10:content.lower().index(address_pb_find.lower())] or "Order of" not in address_pb_find)):
                            ...
                            latitude=""
                            longitude=""
                            address_list.append([address_pb_find,latitude,longitude,city_find,state_find,i])
                        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        ...
        if len(bank) == 1:
            for i in range(0,max(num_banks,num_address_whole)):
                ...
                if num_address_whole > 0:
                    ...
                    zipcode = this_address[5]
                    state = this_address[4]
                    city=this_address[3]
                    longitude=this_address[2]
                    latitude=this_address[1]
                ...
                if num_amount > 0:
                    ...
                    amt = ""
                writer.writerow([photo_id, date, bank_name, zipcode,address,latitude,longitude,city, state, amt])
        else:
            for i in range(0,max(num_banks,num_address_whole)):
                if num_banks > 0:
...
if __name__ == '__main__':
    ...
    headerList = ['Pic ID', 'Date', 'Bank Name', 'Zipcode','Address','Latitude','Longitude','City', 'State', 'Amount']
    with open('checks_april_data'+'.csv','w', newline='', encoding='utf-8') as f1:
    ...
    #     writer.writerow(row)
        ...
        # exit()
        toclean = pd.read_csv('checks_april_data.csv')
        deduped = toclean.drop_duplicates(["Bank Name","Zipcode","City","State","Amount"])
        deduped.reset_index()
        deduped.to_csv('checks_april_data.csv',index=False)
        
        
        
        unique_data = pd.read_csv('checks_april_data.csv')
        address_list=""
        latitude_list=""
        longitude_list=""
        headerList = ['Pic ID', 'Date', 'Bank Name', 'Zipcode','Address','Latitude','Longitude','City', 'State', 'Amount']
        with open('checks_april_data_updated'+'.csv','w', newline='', encoding='utf-8') as f1:
            dw = csv.DictWriter(f1, delimiter=',', fieldnames=headerList)
            dw.writeheader()
            writer=csv.writer(f1, delimiter=',')#lineterminator='\n',
            for i in range(0,len(unique_data["Address"])):
                concat_addr=''
                if(unique_data["Address"][i]!="nan" and type(unique_data["Address"][i])==str):
                    concat_addr=concat_addr+unique_data["Address"][i]
                
                if(unique_data["City"][i]!="nan" and type(unique_data["City"][i])==str):
                    print(type(unique_data["City"][i]))
                    concat_addr=concat_addr+","+unique_data["City"][i]
                if(unique_data["State"][i]!="nan" and type(unique_data["State"][i])==str):
                    concat_addr=concat_addr+","+unique_data["State"][i]
                if(unique_data["Zipcode"][i]!="nan" and type(unique_data["Zipcode"][i])==str):
                    concat_addr=concat_addr+","+unique_data["Zipcode"][i]
                    concat_addr=concat_addr+","+"United States"
                if(concat_addr!=""):
                    url = "https://maps.googleapis.com/maps/api/place/textsearch/json?query="+concat_addr+"&key=AIzaSyDeK7WMVl3E9SShxDRAvT282rUpp4o1D_k"
                    response = requests.request("GET", url, headers=headers, data=payload)
                    response=response.json()
       
                    if(len(response["results"])!=0):
                        latitude_list=(response["results"][0]["geometry"]["location"]["lat"])
                        longitude_list=(response["results"][0]["geometry"]["location"]["lng"])
                        address_list=(response["results"][0]["formatted_address"])
                        print(response["results"][0]["formatted_address"])
                    else:
                        latitude_list=""
                        longitude_list=""
                        address_list=(unique_data["Address"][i])
                else:
                    latitude_list=""
                    longitude_list=""
                    address_list=""
                writer.writerow([unique_data["Pic ID"][i], unique_data["Date"][i], unique_data['Bank Name'][i], unique_data['Zipcode'][i], address_list,latitude_list,longitude_list,unique_data['City'][i],unique_data['State'][i],unique_data['Amount'][i]])
        #Locating address:
